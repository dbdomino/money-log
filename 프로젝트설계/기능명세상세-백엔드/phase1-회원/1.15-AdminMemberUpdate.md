...
> API: `AdminMemberUpdate` · Phase 1 · 기능번호 1.15  
...
| `email` | string \| null | 갱신된 이메일. 요청에서 `null`을 보냈으면 `null`(값을 비운 결과) |
| `phone` | string \| null | 연락처. **하이픈(-) 없이 숫자만** 저장값 |
| `intro` | string \| null | 갱신된 자기소개. 요청에서 `null`을 보냈으면 `null`(값을 비운 결과) |
| `role` | number | 갱신된 권한. `1` 관리자, `3` 일반 |

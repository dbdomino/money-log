# [백엔드] 월별 가계부 목록 조회

> API: `LedgerMonthlyList` · Phase 4 · 기능번호 4.8  
> 인덱스: [README.md](../README.md)

## 메타

| 항목 | 내용 |
|------|------|
| Phase | 4 (가계부·고정지출) |
| 기능 이름 | 월별 가계부 목록 조회 |
| 기능번호 | 4.8 |
| API 이름 | `LedgerMonthlyList` |
| Method | `GET` |
| URL | `/api/v1/ledger/monthly` |
| 권한 | 로그인 |
| Content-Type | — (Body 없음) |

## 요청

### Path Variables

없음 (Query only)

### Query Parameters

| 이름 | 타입 | 필수 | 설명 |
|------|------|:----:|------|
| `year` | number | ✅ | 조회할 가계부 연도 |
| `month` | number | ✅ | 조회할 가계부 월 (1~12) |
| `paymentMethodId` | number | ❌ | 지출·소득·고정지출 **수단**으로 필터 |
| `expendGroupId` | number | ❌ | **지출유형**으로 필터 (지출·고정·할부 행) |
| `dateFrom` | string | ❌ | **결제일(`paymentDate`) 필터 시작일**. 형식 `YYYY-MM-DD`. 이 날짜 **이상**(포함). `year`·`month`로 잡은 달 안에서 추가 좁히기용 |
| `dateTo` | string | ❌ | **결제일(`paymentDate`) 필터 종료일**. 형식 `YYYY-MM-DD`. 이 날짜 **이하**(포함). `dateFrom`과 함께 쓰면 그 사이 결제일만 |
| `keyword` | string | ❌ | 장소·내용 **부분 일치** 검색 |
| `type` | string | ❌ | 행 종류로 필터. `EXPENSE` \| `INCOME` \| `FIXED` \| `INSTALLMENT`. 콤마로 여러 개 지정 가능(예: `EXPENSE,INSTALLMENT`). 생략하면 전부. **고정지출만·일반 내역만** 보고 싶을 때 쓴다 |
| `sort` | string | ❌ | 정렬 기준. `paymentDate`(결제일) \| `amount`(금액). 기본 `paymentDate` |
| `order` | string | ❌ | 정렬 방향. `asc` \| `desc`. 기본 `desc` |

### Headers

| 이름 | 필수 | 설명 |
|------|:----:|------|
| `Authorization` | ✅ | `Bearer <accessToken>` |

### Body

없음

## 응답

공통 래퍼: [_공통.md § 응답 래퍼](../_공통.md#응답-래퍼)  
목록 표준: [_공통.md § 목록 응답 규칙](../_공통.md#목록-응답-규칙) (`data.list` = object 배열)

### 성공 (`resCode: 200`) — `data`

| 필드 | 타입 | 설명 |
|------|------|------|
| `year` | number | 조회 연도 |
| `month` | number | 조회 월 |
| `expenseTotal` | number | 해당 월 지출 합계 (일반+할부+고정) |
| `incomeTotal` | number | 해당 월 소득 합계 |
| `list` | array (object[]) | 통합 목록 (정렬·필터 적용 후) |

#### `list[]` 요소

| 필드 | 타입 | 설명 |
|------|------|------|
| `ledgerItemId` | string | 목록 행 식별자. `EXPENSE`/`INSTALLMENT`는 `expense:{expenseId}`, `INCOME`은 `income:{incomeId}`, `FIXED`는 `fixed:{fixedExpenseId}:{year}:{month}` |
| `type` | string | `EXPENSE` \| `INCOME` \| `FIXED` \| `INSTALLMENT` |
| `sourceId` | number | 원본 PK (`expenseId`, `incomeId`, `fixedExpenseId`). `FIXED`는 `year`·`month`와 함께 써야 월별 고정지출 내역 1행이 특정된다 |
| `paymentDate` | string | 해당 행의 **결제일**(또는 고정지출의 그 달 결제일). 형식 `YYYY-MM-DD`. `dateFrom`/`dateTo`·`sort=paymentDate`의 기준 필드 |
| `amount` | number | 금액 (지출·소득 부호는 type으로 구분) |
| `paymentMethodId` | number \| null | 소득·지출 수단 |
| `paymentMethodName` | string | 수단 이름. **행 종류로 규칙이 갈린다** — `EXPENSE`·`INSTALLMENT`·`INCOME`은 **등록 당시 스냅샷**, `FIXED`는 **조회 시점 현재 이름**이다 |
| `expendGroupId` | number \| null | 지출·고정지출만 |
| `expendGroupName` | string \| null | 지출유형 이름. `INCOME`은 소득에 지출유형이 없어 `null`이다. `paymentMethodName`과 같은 규칙으로 `EXPENSE`·`INSTALLMENT`는 스냅샷, `FIXED`는 현재 이름이다 |
| `place` | string \| null | 장소. `INCOME`·`FIXED`는 그 컬럼이 없어 `null`이다. `keyword` 검색이 보는 두 칸 중 하나 |
| `content` | string \| null | 내용. 네 종류 모두 갖는다. `keyword` 부분 일치 검색이 보는 두 칸 중 하나다(다른 하나는 `place`) |
| `fixedExpenseName` | string \| null | `FIXED`일 때 고정지출 이름 |
| `installmentGroupId` | number \| null | 할부 그룹 |
| `installmentIndex` | number \| null | N번째 |
| `installmentTotal` | number \| null | 총 개월 |

### 실패 — 대표 `resCode`

| resCode | 조건 |
|---------|------|
| 1001 | 로그인 필요 |
| 3501 | year·month 누락·범위 오류 |

## 예시

### 성공

**Request**

```http
GET /api/v1/ledger/monthly?year=2026&month=7&sort=paymentDate&order=desc HTTP/1.1
Host: localhost:8081
Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```

**Response**

```json
{
  "resCode": 200,
  "data": {
    "year": 2026,
    "month": 7,
    "expenseTotal": 650000,
    "incomeTotal": 3500000,
    "list": [
      {
        "ledgerItemId": "income:501",
        "type": "INCOME",
        "sourceId": 501,
        "paymentDate": "2026-07-25",
        "amount": 3500000,
        "paymentMethodId": 3,
        "paymentMethodName": "월급통장",
        "expendGroupId": null,
        "expendGroupName": null,
        "place": null,
        "content": "7월 급여",
        "fixedExpenseName": null,
        "installmentGroupId": null,
        "installmentIndex": null,
        "installmentTotal": null
      },
      {
        "ledgerItemId": "fixed:1:2026:7",
        "type": "FIXED",
        "sourceId": 1,
        "paymentDate": "2026-07-05",
        "amount": 500000,
        "paymentMethodId": 1,
        "paymentMethodName": "국민카드",
        "expendGroupId": 2,
        "expendGroupName": "주거",
        "place": null,
        "content": "원룸 월세",
        "fixedExpenseName": "월세",
        "installmentGroupId": null,
        "installmentIndex": null,
        "installmentTotal": null
      },
      {
        "ledgerItemId": "expense:205",
        "type": "INSTALLMENT",
        "sourceId": 205,
        "paymentDate": "2026-07-01",
        "amount": 100000,
        "paymentMethodId": 1,
        "paymentMethodName": "국민카드",
        "expendGroupId": 5,
        "expendGroupName": "쇼핑",
        "place": "가전매장",
        "content": "노트북 (3/12)",
        "fixedExpenseName": null,
        "installmentGroupId": 50,
        "installmentIndex": 3,
        "installmentTotal": 12
      }
    ]
  }
}
```

### 실패

```json
{
  "resCode": 3501,
  "data": {
    "message": "year와 month는 필수입니다"
  }
}
```

## 비고

- 목록 응답 표준: `data.list` = **object 배열** ([_공통 § 목록 응답 규칙](../_공통.md#목록-응답-규칙)).
- **일반 지출·할부·소득·고정지출**을 한 **응답 목록**으로 합쳐 보여준다. 저장은 서로 다른 테이블이며(월별 지출 내역 / 월별 수입 내역 / 월별 고정지출 내역), 이 API가 조회 시점에 합칠 뿐이다.
- 고정지출 행은 **월별 고정지출 내역**에서 읽는다. 그 연·월의 내역이 아직 없으면 [4.5](./4.5-FixedExpenseMonthlyList.md)와 같은 규칙으로 **처음 조회할 때 만들어 저장**한 뒤 목록에 넣는다.
- 고정지출만 / 일반 내역만 보려면 `type` 필터를 쓰거나, 고정지출은 [4.5 `FixedExpenseMonthlyList`](./4.5-FixedExpenseMonthlyList.md)로 따로 조회한다.
- 이미 연 달에 고정지출을 새로 등록·수정·삭제한 경우, 그 달 목록에 반영하려면 [4.9 `FixedExpenseMonthlySync`](./4.9-FixedExpenseMonthlySync.md)를 먼저 호출한다.
- `FIXED` 행의 `paymentMethodName`·`expendGroupName`은 **스냅샷이 아니라 현재 이름**이다(일반 지출·소득 행은 스냅샷).
- `type=INSTALLMENT`는 할부 지출 건 (`installmentGroupId` 있음).
- Path·Query **혼용 없음** (Query only).
- `dateFrom`/`dateTo`는 행의 **결제일(`paymentDate`)** 범위다. `year`·`month`가 가계부 달을 정하고, 그 안에서 결제일로 더 좁힐 때 쓴다.

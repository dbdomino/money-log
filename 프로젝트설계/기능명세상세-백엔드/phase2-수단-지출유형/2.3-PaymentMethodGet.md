...
> API: `PaymentMethodGet` · Phase 2 · 기능번호 2.3  
...
| `purpose` | string | `EXPENSE`(지출용) \| `INCOME`(소득용). 사용 중 목록(2.6)이 이 값으로 지출용·소득용을 가른다 |
| `inUse` | boolean | 사용 여부. `false`면 지출·소득 입력 화면의 사용 중 목록(2.6)에서 빠진다 |
| `cardExpiry` | string \| null | 카드 유효기간 `YYYY-MM`. `type=ACCOUNT`(계좌)면 항상 `null` |
| `deleted` | boolean | 삭제 표시 여부. `true`여도 행은 남아 관리 목록(2.2)에는 계속 보이며, 사용 중 목록(2.6)에서만 빠진다 |

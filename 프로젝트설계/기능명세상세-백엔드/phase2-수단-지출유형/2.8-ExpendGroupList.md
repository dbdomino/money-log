...
> API: `ExpendGroupList` · Phase 2 · 기능번호 2.8  
...
| `list[].name` | string | 이름 |
| `list[].inUse` | boolean | 사용 여부. `false`면 지출 입력 화면의 사용 중 목록(2.13)에서 빠진다 |
...
| `list[].deleted` | boolean | soft delete 여부. 이 목록은 삭제 표시된 유형까지 전부 돌려주므로, 화면은 이 값으로 살아 있는 유형과 삭제된 유형을 구분한다 |
...
- 이 목록은 **삭제 표시된 유형까지 전부** 돌려준다(관리 목록). 사용 중인 것만 필요하면 2.13을 쓴다.

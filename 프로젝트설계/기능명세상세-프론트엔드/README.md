...
현재 **1.6 권한 없음만 완료**, 나머지 30개는 **골격**이다.

1.6 이 먼저 선 이유는 그 화면이 007 공통 기반의 범위이기 때문이다 — 권한 차단이 착지할 곳이
없으면 진입 판정을 완성할 수 없고, 이 화면만 백엔드를 부르지 않아 다른 화면과 성격이 다르다.
나머지는 008~012 가 자기 범위의 문서를 채운다.

...
> 기준: [2.2. 기능명세 - 프론트엔드.md](../2.2.%20기능명세%20-%20프론트엔드.md) · [2.기능명세.md](../2.0.%20기능명세.md)  
...
현재 **화면 24건이 완료**다 — Phase 1 회원 10건 · Phase 2 수단·지출유형 8건 · **Phase 3
지출·소득 5건과 Phase 5 가계부 1건**. 나머지 7개(Phase 4 고정지출 5건 · Phase 6 목표·통계
2건)는 **골격**이다.

1.6 권한 없음은 007 공통 기반이 먼저 세웠다 — 권한 차단이 착지할 곳이 없으면 진입 판정을
완성할 수 없고, 이 화면만 백엔드를 부르지 않아 다른 화면과 성격이 다르다. Phase 1 의 나머지
9건(1.1~1.5 · 1.7~1.10)은 008 이, Phase 2 의 8건(2.1~2.8)은 009 가, **Phase 3 의 5건
(3.1~3.5)과 Phase 5 의 1건(5.1)은 010 이** 채웠다. 남은 7개는 011·012 가 채운다.

**010 에서 로그인 뒤 착지가 채워졌다.** 008 이 로그인 성공 시 `/ledger` 로 보내는데 그 자리에
그동안 아무것도 없었다 — 5.1 월별 가계부가 서면서 처음으로 실제 화면이 됐다.

**009 가 목록 하나에 모달 셋이라는 모양을 굳혔다.** 2.1~2.4 와 2.5~2.8 이 같은 모양이며,
010~012 의 목록 화면들이 그것을 따른다. **010 은 거기에 셋을 더했다** — 한 목록에 여러
종류를 섞어 보이는 법, 화면이 값을 고쳐 맞추지 않는 규칙, 한 폼에서 통로가 갈릴 때의 판단이다.
공통 규칙은 [_공통.md](./_공통.md) 에 있다.

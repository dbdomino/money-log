...
| **005 고정지출 설정·월별 목록** | ✅ 구현됨 | 6건 (4.1~4.5·4.7) |
| **005 월별 수정·가계부·재작성** | ⏳ **구현 중** | 3건 (4.6·4.8·4.9) — 아직 없다 |
| 006 목표금액·통계 | ⏳ 미착수 | — |

합계 **47개 오퍼레이션 / 31개 경로**(헬스체크 포함).

005 의 남은 3건이 붙으면 `openapi.yaml` 과 `OpenApiDocumentIT.REPRESENTATIVE_PATHS` 를 함께 늘린다. `openapi.yaml` 을 잊으면 `OpenApiDocumentIT` 가 먼저 깨진다.

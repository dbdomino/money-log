...
| `{ resCode, data }` · `1001` · `1006` | 백엔드가 이미 확정한 **응답 계약**(헌장 원칙 III)이다. 코드마다 화면 모듈의 반응이 달라 이름 없이는 FR-611·612 를 적을 수 없다 |
...
- **화면 1.6 을 007 이 만드는 것**이 Phase 구분(1.6 은 Phase 1)과 어긋나 보인다. 권한 차단이 착지할 곳이 없으면 FR-615 를 완성할 수 없고 이 화면만 API 를 부르지 않는다 — 범위 표와 본문에 근거를 적어 두었다.

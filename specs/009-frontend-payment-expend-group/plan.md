...
- **008 의 템플릿 한 곳은 고친다.** 모달을 여는 스크립트를 조각으로 뽑아 008 의 관리자 회원
  화면도 그것을 쓰게 한다. 009 가 화면 둘을 더하고 010~012 가 넷을 더 얹으므로, 지금 뽑지
  않으면 같은 스크립트가 여섯 벌이 된다. **008 은 우리가 만든 것이라 "007 을 고치지
  않는다"에 걸리지 않으며**, 008 의 시험이 그대로 통과하는 것으로 동작이 같음을 확인한다.
...
부르는 API 11건 · 새 에러코드 0건 · DB 테이블 0개 · 새 의존 0건 ·
**007 을 고치는 파일 1개**(호출 통로) · **008 을 고치는 파일 1개**(관리자 회원 템플릿).
...
├── layout/                           (007)
├── fragments/                        (007)(008)
│   ├── status-badge.html             ← 009: 사용 여부와 삭제 여부를 그리는 조각 둘
│   └── modal-open.html               ← 009: 서버가 정한 열 모달을 007 스크립트에 넘긴다
├── auth/ · member/                   (008)
├── admin/members.html                (008) ← 009: 위 modal-open 조각을 쓰게 바꾼다
...
| **008 의 관리자 회원 템플릿을 고친다** (009 가 앞선 기능의 산출물을 건드린다) | 모달을 여는 스크립트가 008 에 인라인으로 들어 있다. 009 가 화면 둘을 더하고 010~012 가 넷을 더 얹으므로, 조각으로 뽑지 않으면 **같은 스크립트가 여섯 벌**이 되고 한 벌만 고친 버그가 나머지에서 계속 산다 | **009 만 조각을 쓰고 008 은 그대로 둔다** — 같은 일을 하는 두 방식이 공존하게 되고, 다음 사람이 어느 쪽을 따라야 할지 알 수 없다. **지금 뽑지 않고 010 에서 뽑는다** — 그때는 네 벌이 되어 있어 옮길 자리가 더 많다 |

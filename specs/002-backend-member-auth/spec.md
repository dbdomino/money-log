...
- **FR-106**: 회원가입(1.2)과 관리자 회원 추가(1.12) **양쪽 모두** 성공 시 **기본 지출유형 10종**(식비·교통·주거·통신·쇼핑·장보기·의료·교육·문화·기타)을 `defaultGroup=true`로 생성하고, 30×30 기본 아이콘 PNG를 회원별 복사본으로 만들어야 한다. 복사본 이름은 **`{id_key}_{expendGroupId}.png`** 이며 `{id_key}`는 `tbl_user`의 대리키, `{expendGroupId}`는 방금 만든 `tbl_user_expend_group.idx`다(`003`의 FR-224와 같은 규칙이다). **유형 이름과 로그인 아이디는 파일명에 쓰지 않는다** — 파일명이 조회 URL 경로에 그대로 실리므로 이름을 넣으면 `/`·`..`·공백으로 경로를 벗어나거나 인코딩이 어긋나고, 아이디를 넣으면 자식 데이터에 로그인 아이디를 복사하는 것이 된다. 파일명에 PK가 들어가므로 **행을 먼저 저장해 `idx`를 받은 뒤 파일을 복사한다.** DB에는 파일명만 저장하고 조회 경로·Base URL은 저장하지 않는다
...
- **FR-121**: 감사 컬럼 `created_by`/`updated_by`에는 **그 행을 만들거나 고친 주체의 `id_key`** 를 남겨야 한다. 관리자가 다른 회원을 추가·수정하면 관리자의 `id_key`, 회원이 자기 정보를 고치면 그 회원 본인의 `id_key`다. **`null`은 본인 가입뿐이다** — 그 시점에는 자기 `id_key`가 아직 없고, 그래서 `tbl_user`만 두 컬럼이 nullable이다
  - **개정(2026-09-06)**: 원래 "본인 가입·본인 <b>수정</b>은 `null`"로 적었으나, 본인 수정을 `null`로 남기려면 감사자 공급자가 "이 행의 주인이 요청자와 같은가"를 알아야 한다. `AuditorAware`는 대상 행을 모르고 현재 요청의 주체만 아는 자리라 그 판정을 넣을 수 없다. 게다가 `null`은 "누가 고쳤는지 모른다"로 읽히는데 실제로는 알고 있다 — 값을 남기는 편이 감사 기록으로서 더 정확하다. 가입만 `null`인 것은 기술적 제약이자 사실이다
...
| `1006` | 세션 무효(로그아웃·중복 로그인·해시 불일치) | 1.4·1.6·1.7 |

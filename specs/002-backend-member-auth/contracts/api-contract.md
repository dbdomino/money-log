...
| 다른 곳에서 로그인해 세션이 교체됐다 | `1005` (아래 개정 참고) |

> **개정(2026-09-06) — 갱신에서 `1006`은 나올 수 없다.**
> 원래 이 표는 "다른 곳에서 로그인해 세션이 교체된 경우"를 `1006`으로 적었으나, 그 판정은
> 구현이 만들어 낼 수 없다. 갱신은 **Refresh 해시로만** 세션을 찾는데(§3 2단계), 세션 폐기는
> 두 해시를 `NULL`로 만든다(FR-111). 교체된 세션은 그 순간부터 해시로 찾을 수 없으므로
> "교체된 세션의 옛 토큰"과 "아무 문자열"이 서버에게 똑같이 보인다.
>
> 둘을 구분하려면 폐기된 토큰의 해시를 어딘가 남겨야 하는데, 그것은 FR-111이 막는 것이다
> (해시를 지우는 것이 실질적 무효화다). 구분해서 얻는 것도 없다 — 어느 쪽이든 클라이언트가
> 할 일은 재로그인 하나다. 오히려 "당신 세션이 다른 곳에서 교체됐다"를 알려 주면, 훔친
> 토큰을 들고 온 쪽에게 그 계정이 살아 있다는 사실을 확인해 주는 셈이 된다.
>
> **`1006`은 Access Token 경로에만 남는다.** 그쪽은 토큰의 `sid`로 세션 행을 찾을 수 있어
> "세션이 무효"와 "토큰이 잘못됨"이 실제로 구분된다.
...
| 상황 | 코드 | 적용 API |
|---|---|---|
| 이메일 형식이 어긋남 · 필수 필드 누락 | `9001` | 1.9·1.10·1.11 |
| 형식은 맞는데 일치하는 회원이 없음 | `2001` | 1.9·1.10·1.11 |
| `memberId`+`nickname` 불일치 | `2001` | 1.10·1.11 |
| 대상 회원이 비활성 | `1004` | **1.10·1.11만** |

**비활성 판정은 1.9 에 적용하지 않는다.** spec.md 의 에러코드 표도 `1004` 의 사용 지점에
1.9 를 넣지 않았고, `1.9-MemberFindId.md` 의 실패 표는 `2001`·`9001` 둘뿐이다. 아이디 찾기는
"이 이메일로 가입한 아이디가 무엇인가"에만 답하며, 그 계정이 지금 로그인할 수 있는지는
로그인이 판정한다 — 정지된 회원에게 자기 아이디조차 알려 주지 않을 이유가 없다.
(이 표가 1.9~1.11 을 한 묶음으로 적어 오해를 부르던 것을 2026-09-06 에 API 별로 갈랐다.)

...
3. **Given** 등록·수정 요청에 허용 범위를 벗어난 아이콘 파일을 실으면, **When** 저장하면, **Then** `3102`로 거부된다
...
8. **Given** 유형 이름을 바꿔도, **When** 아이콘을 조회하면, **Then** 파일명은 **생성 시점 이름 그대로**라 기존 파일을 계속 찾는다
...
- 이미 삭제 표시된 지출유형을 다시 삭제하면? [NEEDS CLARIFICATION: 2.12 설계 명세의 실패 코드가 `3103`·`3106`·`3107` 셋뿐이라, 수단의 `3004`에 대응하는 코드가 지출유형 대역에 배정되어 있지 않다. 멱등 성공으로 둘지 새 코드를 배정할지 결정이 필요하다]
- 지출유형 관리 목록(2.8)이 삭제 표시된 유형을 돌려줄 때 프론트는 어떻게 구분하는가? [NEEDS CLARIFICATION: 수단 목록(2.2)에는 `list[].deleted`가 있는데 2.8 응답 필드에는 없어, FR-207대로 삭제분을 내려보내도 화면이 구분할 수 없다. 2.8에 `deleted` 필드를 추가하는 개정이 필요하다]
- 삭제 표시된 수단·유형을 수정할 수 있는가? — **가능하다**(이름 정리 등)
- 아이콘 파일명이 `{id_key}_{유형이름}.png`인데 유형 이름에 파일명으로 못 쓰는 문자가 있으면? [NEEDS CLARIFICATION: 유형 이름의 허용 문자 범위와 파일명 변환 규칙이 명세에 없다]
- 아이콘 업로드의 최대 크기·허용 확장자는? — 위반은 `3102`로 거절한다(FR-219). [NEEDS CLARIFICATION: 설계 명세가 허용 형식을 "png, jpg, gif 등"으로만 적어 최대 바이트 수와 확장자 목록이 확정되지 않았다]
...
- **FR-219**: 아이콘 파일의 형식·크기가 허용 범위를 벗어나면 `3102`로 거절해야 한다
...
- **아이콘 파일**: 회원별 복사본. 이름 규칙 `{id_key}_{유형이름}.{확장자}`이며 `{id_key}`는 `tbl_user`의 대리키다. 로그인 아이디 `user_id`를 파일명·자식 테이블 값에 쓰지 않는다. DB `tbl_user_expend_group.icon_filename`에는 파일명만 저장하고 경로·Base URL은 저장하지 않는다

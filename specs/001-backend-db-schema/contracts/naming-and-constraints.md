...
지출유형 이름 유일성은 삭제 표시된 행을 **포함**한다(`WHERE` 조건이 없는 통 유니크다). 삭제한 이름을 다시 쓰려는 요청을 막는 것이 목적이며, `003`의 FR-209가 같은 규칙을 API 쪽에서 다시 확정했다 — 삭제 표시는 행을 남기는 조치이므로 그 행이 점유한 이름도 계속 점유 상태로 본다.

> **근거 개정(2026-09-06)**: 이 제약의 원래 근거는 "부분 유니크로 삭제분을 빼면 아이콘 파일명 `{user_id}_{유형이름}.png`가 충돌한다"였다. `003`의 clarify에서 **파일명에서 유형 이름을 뺐으므로**(FR-224 — 파일명은 `{id_key}_{expendGroupId}.{확장자}`) 그 근거는 더 이상 성립하지 않는다. **제약 자체는 그대로 유지한다** — 이미 DB에 반영된 `ux_user_expend_group_name (id_key, name)`이고, `003`의 FR-209·US3 시나리오 3이 이 동작을 요구한다. 근거가 사라졌다고 제약을 바꾸면 스키마 변경이 되며 그때는 원칙 VI에 따라 덤프를 재생성해야 한다.

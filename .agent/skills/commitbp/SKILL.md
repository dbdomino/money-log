...
description: >-
  ...
  2–3 Korean lines, creates a git commit with an optional user-supplied title
  prefix such as a ticket id, and then pushes to the remote. Self-contained —
  does not read any other skill. Use when the user asks to commit AND push
  (e.g. "/commitbp", "/commitbp [SKSIAP-1007]", "커밋하고 푸시",
  "커밋 후 푸시까지").
---

# commitbp — 변경이력 취합 요약 커밋 + 푸시

변경이력을 취합해 2~3줄로 요약하고 **커밋한 뒤 원격에 푸시**한다.
1~6단계가 커밋, 7단계가 푸시다. **스킬에 푸시가 없다고 생략하지 않는다.**

> 이 파일 하나로 완결된다. **다른 스킬·규칙 문서를 읽지 않는다.**

## 사용법

- `/commitbp` 와 **같은 메시지**에 커밋 제목 앞에 붙일 **접두사**를 적는다.
  예: `/commitbp [SKSIAP-1007]`
...
- 애매한 파일이 있으면 **커밋 전에** 물어본다. 푸시까지 가는 흐름이라 여기서 걸러야 한다.
...
- 최근 커밋들이 **제목 한 줄만** 쓰는 저장소면 본문을 억지로 붙이지 않는다.
...
- `git log -1 --stat` 으로 확인한다. **커밋이 실패했거나 훅에서 막혔으면 푸시로 넘어가지 않는다.**
...
```bash
git status --porcelain                                 # 미커밋 변경 확인
git rev-parse --abbrev-ref HEAD                        # 현재 브랜치
git rev-parse --abbrev-ref --symbolic-full-name @{u}   # 업스트림 (없으면 에러)
git remote -v
```

- 커밋에 넣지 않기로 한 변경이 남아 있는 건 정상이다. 다만 **커밋했어야 할 변경이 남아 있으면 푸시하지 않고 알린다.**
- 기본·보호 브랜치(`main`, `master`, `develop`, `release/*`)에 있으면 **푸시하지 않고 먼저 사용자에게 확인**한다.
- 업스트림이 있으면 `git push`, 없으면 `git push -u origin <현재브랜치>` — `--set-upstream`이 필요하면 사용자에게 확인한다.
- **`--force` / `--force-with-lease` 는 쓰지 않는다.** 사용자가 명시적으로 요청한 경우에만.
- 푸시가 거절되면(non-fast-forward·권한·충돌) **자동으로 pull/rebase/force 하지 않는다.** 오류 메시지를 그대로 전달하고 어떻게 할지 묻는다.
- 인증 프롬프트로 막히면 사용자에게 `! git push` 로 직접 실행하도록 안내한다.

푸시 후 브랜치·원격·커밋 제목을 한 줄로 보고한다. **PR 생성은 요청 없이 하지 않는다.**
...
- 비밀·토큰·자격증명·대용량 생성물(`build/`, `.gradle/`, `node_modules/` 등)을 커밋·푸시하지 않는다. 발견하면 멈추고 알린다. **푸시된 비밀은 회수 불가**다.
- 사용자가 "커밋만" 요청하면 7단계를 건너뛰고 커밋에서 멈춘다. "커밋 메시지만" 요청하면 메시지만 제시한다.
- `--no-verify`, `--amend`(이미 푸시된 커밋), `git reset --hard` 는 사용자가 명시적으로 요청할 때만. 훅(pre-commit/pre-push)이 실패하면 우회하지 말고 원인을 고친다.

...
| ④ | 가계부 | 지출(일시불·할부)·소득 CRUD, 월별 통합 목록, 고정지출(관리 + 월별 내역), 엑셀 일괄 등록(.xlsx, 최대 300행) |
...
개요: [프로젝트설계/2.기능명세.md](프로젝트설계/2.0.%20기능명세.md)
...
| 기능 | 범위 | 상태 |
|------|------|------|
| 007 | 공통 기반 — 백엔드 호출 통로 · 로그인 세션 · 진입 판정 · 레이아웃 둘 · 모달 껍데기 · 권한 없음 화면 | 완료 |
| 008 | 회원·인증 화면 9건 — 로그인 · 가입 · 아이디 찾기 · 비밀번호 찾기·변경 · 본인 정보 · 관리자 회원 목록·추가·수정 | 완료 |
| 009 | 수단·지출유형 화면 8건 — 목록 둘과 그 위의 등록·상세·수정 모달 여섯 | 완료 |
| 010 | 가계부·지출·소득 화면 6건 — 월별 가계부와 그 위의 등록·수정 모달 넷 · 엑셀 일괄 등록 | 완료 |
| 011~012 | 고정지출 · 목표·통계 화면 7건 | 예정 |

010 까지 끝나 **화면 24건이 서고, 로그인 뒤 착지가 처음으로 실제 화면이 됐다** — 008 이
로그인 성공 시 보내던 `/ledger` 에 그동안 아무것도 없었다. 이제 가입에서 지출·소득 기록까지
화면으로 끝까지 돈다.

009 가 세운 **목록 하나에 모달 셋**이라는 모양을 010 이 따르며 셋을 더했다 — **한 목록에
여러 종류를 섞어 보이는 법**(뱃지가 먼저, 비는 칸은 비워 둔다), **화면이 값을 고쳐 맞추지
않는 규칙**(합계는 필터와 무관, 이름 규칙은 종류마다 갈린다), **한 폼에서 통로가 갈릴 때의
판단**(접는 것은 브라우저, 가르는 것은 서버). 011·012 가 그것을 이어 쓴다.
...
| 3 | `sql/03_create_schema.sql` | `search_path` 설정 (`moneylogdb` 접속 후 실행). **스키마 `moneylog` 자체와 테이블·제약·인덱스·시퀀스·주석은 앱이 만든다** — 백엔드를 한 번 기동하면 끝난다. 이 스크립트는 psql로 직접 붙어 조회할 때만 필요하다 |
...
| 2 | [기능명세](프로젝트설계/2.0.%20기능명세.md) | **최신** |
...
- [2.기능명세](프로젝트설계/2.0.%20기능명세.md): 프론트/백엔드 분리 아키텍처, API 54개, 구현 Phase 정의

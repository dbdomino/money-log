...
- **세션 기반** 로그인 (토큰 로그인·관리자 타인 가계부 조회는 이후 버전)
...
| # | 도메인 | 핵심 내용 |
|---|--------|-----------|
| ① | 회원 관리 | 가입·로그인(세션)·id/pw 찾기·본인 정보 수정·비활성화. 관리자(1)만 회원 목록·추가·타 회원 수정 |
...
| 문서 | 내용 |
|------|------|
| [2.1. 기능명세 - 백엔드.md](./프로젝트설계/2.1.%20기능명세%20-%20백엔드.md) | REST API 51개, Method·URL·구현 Phase |
...
- 인증: 세션 쿠키. 본인 데이터만 접근, 관리자 API는 권한 `1` 필요
...
- [2.기능명세](./프로젝트설계/2.기능명세.md): 프론트/백엔드 분리 아키텍처, API 51개, 구현 Phase 정의
